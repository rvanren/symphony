...
from symphony_model_checker.symphony.value import *
from symphony_model_checker.symphony.bag_util import *
...
class AssertOp(Op):
    ...
    def tladump(self):
        ...
        msg = '"Symphony Assertion (file=%s, line=%d) failed"'%(file, line)
        if self.exprthere:
    ...
    def eval(self, state, context):
        ...
        if not cond:
            (lexeme, file, line, column) = self.token
            context.failure = "Symphony Assertion (file=%s, line=%d) failed"%(file, line)
            if self.exprthere:
