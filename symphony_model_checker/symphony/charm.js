...
function explain_issue(issue) {
    ...
    if (issue.startsWith("Data race ")) {
        ...
        alert("The Symphony program suffers from a data race on variable '" + v + "'." + x);
    }
    else {
        var e = issue_explanation.get(issue);
        if (e == undefined) {
            alert("The Symphony program is not correct." + x);
        }
        else {
            alert(e + x);
        }
...
function explain_turn() {
    alert(`In a Symphony program, threads take turns executing.  This table provides some information on each of these turns.  The turn that is currently active is highlighted in lightgreen, as is the corresponding thread in the thread table below.`);
}
