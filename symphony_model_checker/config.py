...
class SettingsManager:
    def __init__(self) -> None:
        self.user_dir = self._setup_symphony_dir()
    ...
    def _setup_symphony_dir(self):
        ...
        user_dir = pathlib.Path(xdg_config_home, 'symphony-model-checker').expanduser()
        ...
        with self.settings_file.open('r') as f:
            try:
                ...
                if v is not None and isinstance(v, bool):
                    return Setting(disable_web=v)
            except TypeError:
                pass
            return _make_new_settings()

    def _update_from_env(self):
        val = os.environ.get('SYMPHONY_DISABLE_WEB', None)
        if val is not None:
