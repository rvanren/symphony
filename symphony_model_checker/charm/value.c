// This module deals with Symphony values.  A Symphony value is a 64-bit entity
...
// of Symphony values (sorted in case of a set).  If it's a dictionary,
...
// This function pretty-prints Symphony value v in the given string buffer
...
// This function pretty-prints Symphony value v and returns it in a malloc-ed
...
// Print Symphony value v encoded as a JSON object in the given string buffer
...
// Encode the given Symphony value v as a JSON value, returned as a string.
char *value_json(hvalue_t v){
    struct strbuf sb;
    strbuf_init(&sb);
    strbuf_value_json(&sb, v);
...
static hvalue_t value_dict(struct allocator *allocator, struct dict *map){
    ...
    // vals is sorted already by symphony compiler
...
// This function takes a JSON dictionary value and turns it into a Symphony value.
hvalue_t value_from_json(struct allocator *allocator, struct dict *map){
...
// a Symphony thread).  Detects stack overflow.
...
// A Symphony state contains a bag (multiset) of contexts.  This function checks
...
// This function takes a Symphony value, represents it as a string, and then
