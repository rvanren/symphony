// This module implements all the functionality for Symphony VM operations.
...
// Each Symphony N-ary function has a name and a C function that implements it
struct f_info {
    char *name;
    hvalue_t (*f)(struct state *state, struct step *step, hvalue_t *args, unsigned int n);
};

// Bounded variables occur in various Symphony expressions, e.g.:
//      let x, (y, z) = 1, (2, 3):
//      for i, j in ...:
//      def f(x, y):
...
// arg is a Symphony value.  This function matches the value with the bound
...
void op_Assert(const void *env, struct state *state, struct step *step){
    ...
    if (v == VALUE_FALSE) {
        ...
        value_ctx_failure(step->ctx, step->allocator, "Symphony assertion failed");
    }
    else {
        if (step->keep_callstack) {
...
void op_Assert2(const void *env, struct state *state, struct step *step){
    ...
    if (v == VALUE_FALSE) {
        ...
        value_ctx_failure(step->ctx, step->allocator, "Symphony assertion failed: %s", p);
        free(p);
    }
    else {
        step->ctx->pc++;
...
// This is a helper function to call a method.  Symphony supports method
...
// of the thunk.  Here, 'av' is the original address, 'root' is the Symphony
...
void do_Load(struct state *state, struct step *step,
    ...
    if (k != size) {
        ...
        // same functionality in Symphony, we detect if the program is trying
...
// simple Symphony programs.  However, if there is no argument, there is
...
// value is the same as the constant.  That is, Symphony allows 3 = 3, but
...
// names (f_Y).  Also initialize a bunch of handy Symphony value constants.
