...
import symphony_model_checker
from symphony_model_checker.config import settings
from symphony_model_checker import charm # type: ignore
...
from symphony_model_checker.symphony.genhtml import GenHTML
from symphony_model_checker.symphony.brief import Brief
from symphony_model_checker.symphony.verbose import Verbose
from symphony_model_checker.compile import do_compile


args = argparse.ArgumentParser(
    "symphony", description="Symphony programming language compiler and model checker")
...
def handle_hny(ns, output_files, parse_code_only, filenames):
    ...
    print("* Phase 1: compile Symphony program to bytecode", flush=True)
    ...
    except (SymphonyCompilerErrorCollection, SymphonyCompilerError) as err:
        if isinstance(err, SymphonyCompilerErrorCollection):
            errors = err.errors
        else:
            errors = [err.token]
    ...
    if output_files["tla"] is not None:
        with open(output_files["tla"], "w", encoding='utf-8') as f:
            legacy_symphony.tla_translate(f, code, scope)

    if output_files["tex"] is not None:
        with open(output_files["tex"], "w", encoding='utf-8') as f:
            legacy_symphony.tex_output(f, code, scope)
...
def handle_hvm(ns, output_files, parse_code_only, code, scope):
    ...
    if code is not None:
        with open(output_files["hvm"], "w", encoding='utf-8') as fd:
            legacy_symphony.dumpCode("json", code, scope, f=fd)
...
def handle_version(_: argparse.Namespace):
    print("Version:", symphony_model_checker.__package__,
          symphony_model_checker.__version__)
    return 0
...
def main():
    ...
    if len(ns.args) != 1:
        print(f"symphony: error: invalid number of arguments ({len(ns.args)}). Provide 1 argument.")
        args.print_help()
        return 1

    filename = pathlib.Path(ns.args[0])
    if not filename.exists():
        print(f"symphony: error: file named '{filename}' does not exist.")
        return 1
    ...
    # Handle different Symphony compilation stages
    if input_file_type == ".hny":
        ...
        if charm_flag:
            ...
            legacy_symphony.dumpCode(print_code, code, scope)
