...
def load_test_cases() -> List[TestCase]:
    modules_dir = pathlib.Path("symphony_model_checker") / "modules"
    ...
    for f in modules_dir.glob("*.hny"):
        ...
        test_cases.append(TestCase(filename=str(f), symphony_args=['--noweb']))

    with runall_script.open("r") as f:
        # Exclude the first arg, i.e. the symphony script filename
        all_tc = [l.strip().split() for l in f.readlines() if l.startswith("./symphony")]
        for tc in all_tc:
...
def dump_result(result: ComparisonResult, f: TextIO):
    ...
    f.write('---')
    f.write('\n')
    f.write(f'## {" ".join(test_case.symphony_args or [])} {test_case.filename}\n')
    f.write('\n')
    f.write("### Summary\n\n")
...
def evaluate_test_case(test_case: TestCase, n: int) -> TestResult:
    symphony_args = ' '.join(test_case.symphony_args or [])
    filename = test_case.filename
    print(f'Evaluating `./symphony {symphony_args} {filename}`')
    ...
    for _ in range(n):
        ...
        result = subprocess.run(
            f'./symphony -A {symphony_args} {filename}'.split(),
        ...
        baseline_result = subprocess.run(
            f'./symphony -f -A {symphony_args or ""} {filename}'.split(),
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            encoding='utf8'
        )
...
def expected_outputs_match(
    ...
    if isinstance(test_case.lines_to_check, list):
        ...
        if prev_expected_output != curr_expected_output:
            print(
                f'{" ".join(test_case.symphony_args or [])}: Previous and current expected '
...
def merge_expected_outputs(
    ...
    print(f'{symphony_args}: difference on expected_output')
    ...
    print(curr)
    print('```')
    with (results_dir / f"{basename}-{symphony_args}-prev".replace(" ", "_")).open("w") as f:
        f.write(prev)
    with (results_dir / f"{basename}-{symphony_args}-next".replace(" ", "_")).open("w") as f:
        f.write(curr)
    return False
...
def merge_average_durations(
    ...
    print(f'{" ".join(test_case.symphony_args or [])}: difference on average duration by {percent_difference}% > 100%')
