...
class SymphonyDenter(ModifiedDenterHelper):
    def __init__(self, lexer, nl_token, colon_token, indent_token, dedent_token, ignore_eof):
        ...
        self.lexer: SymphonyLexer = lexer

    def pull_token(self):
        return super(SymphonyLexer, self.lexer).nextToken()

denter = None
def nextToken(self):
    if not self.denter:
        self.denter = self.SymphonyDenter(self, self.NL, self.COLON, SymphonyParser.INDENT, SymphonyParser.DEDENT, ignore_eof=False)
